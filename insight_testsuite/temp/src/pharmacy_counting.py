    ...
    for i, line in enumerate(f):
        ...
        if len(line) == 5:
            ...
            if len(line) == 5:
                update_dicts(line)
                if ',' in line[drug]:
                    # track drug names with commas
                    bad.add(line[drug])
            else:
                sys.stderr.write(f'row {i} of file not parsed correctly')
...
# get all drugs in the strange case the keys are not identical sets
seq = set(scrib.keys()).union(costsum.keys())
if (len(seq) != len(scrib.keys())) | (len(seq) != len(costsum.keys())):
    sys.stderr.write(f'Warning: drug cost and drug prescribers not recorded properly')   
...
def write_line(drug, no_quote = True):
    try:
        ...
        sys.stderr.write(f'error in retrieving records for drug {drug}')  
